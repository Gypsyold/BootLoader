	...
	else if(BootStaFlag & IAP_XMODEMD_FLAG)
	{
		...
		if((datalen == 1)&&(data[0] == 0x04))
		{
			...
			if((UpDataA.XmodemNB % (STM32_PAGE_SIZE / 128)) != 0)
			{
				...
				else STM32_WriteFlash(STM32_A_SADDR + ((UpDataA.XmodemNB / (STM32_PAGE_SIZE / 128)) ) * STM32_PAGE_SIZE,(uint32_t *)UpDataA.UpdataBuff,(UpDataA.XmodemNB % (STM32_PAGE_SIZE / 128)));
			}
